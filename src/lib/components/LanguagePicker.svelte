<script lang="ts">
	...
	let { codes = $bindable([]), pending = $bindable('') }: { codes: string[]; pending?: string } =
		$props();
	let q = $state('');
...
<div class="relative">
	<div class="flex flex-wrap gap-1 mb-1">
		{#each codes as code (code)}
			<span
				class="inline-flex items-center gap-1 border border-slate-300 rounded-full bg-white px-2 py-0.5 text-xs"
			>
				{languageName(code)}
				<button
					type="button"
