...
export const openLibrary: Provider = {
	...
	async fetch(isbn13, f) {
		...
		if (Array.isArray(ol.publish_places)) c.published_place = ol.publish_places.join(', ');
...
export const bibbi: Provider = {
	...
	async fetch(isbn13, f) {
		...
		if (Array.isArray(w.creator)) {
			const names = w.creator.map((a: any) => reorderName(a.name ?? '')).filter(Boolean);
			if (names.length) c.authors = names;
		}
...
export function parseRefworks(block: string, isbn13: string): Candidate | null {
...
export const libris: Provider = {
	name: 'Libris',
	async fetch(isbn13, f) {
		const res = await f(`https://libris.kb.se/xsearch?query=NUMM:${isbn13}&format=refworks`);
		if (!res.ok) return null;
...
const DEFAULT_PROVIDERS: Provider[] = [libris, openLibrary, bibbi];
...
export async function gatherCandidates(
	...
	const settled = await Promise.allSettled(providers.map((p) => p.fetch(isbn13, f)));
	const out: Candidate[] = [];
