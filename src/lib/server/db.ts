...
import { classifyQuery, formatContributors, formatLanguages, type Contribution } from '../search';
...
export async function suggestSubjects(
	...
	if (!q) return [];
	const r = await db
		.prepare(`SELECT name FROM subject WHERE name_folded LIKE ?1 ORDER BY name LIMIT ?2`)
		.bind(`%${q}%`, limit)
		.all<{ name: string }>();
...
export async function searchBooks(db: D1Database, raw: string): Promise<ResultBook[]> {
