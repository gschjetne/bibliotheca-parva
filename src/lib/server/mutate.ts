...
export async function updateBook(db: D1Database, id: number, input: BookInput): Promise<void> {
	const v = bookColumnValues(input);
	await db
...
export async function getBookForEdit(db: D1Database, id: number): Promise<EditableBook | null> {
	const book = await db
